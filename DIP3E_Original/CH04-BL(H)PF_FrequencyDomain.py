...
def main_demo():
    figure = plt.figure()
    im = cv2.imread('DIP3E_Original_Images_CH04/Fig0445(a)(characters_test_pattern).tif')
...
def main_apply():
    figure = plt.figure()
    im = cv2.imread('DIP3E_Original_Images_CH04/Fig0457(a)(thumb_print).tif')

    plot_spectrum = figure.add_subplot(131)
    plot_spectrum.set_xticks([])
    plot_spectrum.set_yticks([])
    fft = fft_im(im, plot_spectrum)
    # 高通滤波
